> **Mirrored copy, full text, for Codex.** Written 2026-08-06 by the AI Quote
> Tool's assistant. Canonical source is the `yll-quote-tool` repo:
...
> or on branch `claude/copilot-labor-tracking-a14bf3` if that PR hasn't merged
> yet). If the two copies ever drift, the quote tool's copy wins — update this
> one to match, don't edit this one as the source of truth.
>
> **Repo split, so this makes sense as you read it:** the quote tool is the
> engine and system of record (jobs, budgeted hours, pay math, the Telegram
> bot, APIs). **This repo (yll-call-copilot) is the Operations Hub** — the
> crew- and office-facing screens (clock in/out, my hours, my P4P earnings,
> leaderboard) that call the quote tool's APIs. See A4 below for the full
> split, and A8 for the draft API contract this hub will consume.
>
> This repo's own hub plan/build doc isn't visible to the quote tool side yet —
> point back to it (a file here, or paste it into chat) so the two plans can be
> reconciled into one contract doc at `docs/context/OPERATIONS_HUB_CONTRACT.md`
> in the quote tool repo.
...
> Software, the quote tool codebase).
> **Part C** is the review record: what five reviewers found and what was done
> about each finding.
> Related: `project_inventory_system.md` (#82, in the quote tool repo) and this
> repo (the YLL Operations Hub).
...
screens land in the YLL Operations Hub (this repo, yll-call-copilot), which reads
...
- Every crew-facing surface (my-earnings, leaderboard, bot nudges) MUST render
  ...
  **This is the rule that matters most for this repo's UI code.**
...
- **Quote tool:** system of record and engine. Tables, BH math, labor-revenue
  math, pay math, scheduling data, office screens, the Telegram bot, and
  token-authed HTTP APIs for the hub.
- **YLL Operations Hub = this repo (yll-call-copilot), extended.** Not a new
  repo. Crew clock-in, my-hours, my-stats, my-earnings live here, alongside the
  existing sales/coach app. Later: the yard-sign/door-hanger team (out of scope
  here).
- **Data wiring:** this hub calls quote tool APIs. Each app keeps its own
  database. Verified 2026-08-06: the two already run separate Supabase projects
  (this repo's is `mjmociuxxxwxvasnpxav`, the quote tool's is
  `chhntsbnbofyqrpivuog`), so this matches reality with zero migration.
- **Division of labor:** Codex works full-stack on hub features, including the
  quote tool API routes its screens need. Two assistants in one repo (the quote
  tool), so:
  - Codex reads the quote tool repo's `AGENTS.md` and follows it as a third dev:
    branch prefix `codex/...`, PR into master, gates green
    (`npx tsc --noEmit · npm run lint · npm test`), a human merges, never merge
    stale.
  - **The quote tool's AGENTS.md needs a real ownership row for Codex** (its
    table names only Jason and Naldo) with a named human reviewer for Codex PRs.
    Open, A6.
  - **Schema migrations for the labor tables (`crew_members`,
    `job_time_entries`, job BH/labor-revenue columns) have ONE author: the
    quote tool's assistant.** Codex consumes via API and never migrates these
    tables. Prevents two assistants racing the same first migration.
  - The contract doc is the handshake: endpoints, auth, and shapes are written
    there BEFORE either side builds. Home:
    `docs/context/OPERATIONS_HUB_CONTRACT.md` in the quote tool repo (canonical)
    with a pointer copy in this repo's `docs/`.
...
- Operations Hub my-stats (hours, efficiency, provisional vs earned pay),
  leaderboard, bot nudges. **This is this repo's phase.**
...
11. **AGENTS.md ownership row for Codex** (in the quote tool repo) with a named
    reviewer, before parallel work starts.
...
3. Get Codex's hub plan doc surfaced, then write
   `docs/context/OPERATIONS_HUB_CONTRACT.md` in the quote tool repo from both
   plans and mirror a pointer into this repo's `docs/`.
4. Ledger row for the initiative and per-phase rows when building starts (quote
   tool repo's task ledger).
...
## A8. What this hub needs from the quote tool (draft API surface)

Starting list for the contract doc, so Codex has something concrete to react to.
All endpoints token-authed, all added to the quote tool's `operatorGate`
allowlist in the same PR that creates them, all verified logged-out.
...
| `GET /api/ops/schedule` | week view if this hub renders it | 3 |

**Non-negotiables for the contract doc:**

- **Identity:** `crew_members` (Phase 1, quote tool side) holds the quote-tool
  id, the Telegram user id, and this hub's auth id. One mapping table, one
  owner.
...
- **`me/earnings` must return provisional and earned separately** and this hub
