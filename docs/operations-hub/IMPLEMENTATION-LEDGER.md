...
| Recording sync, performance-only metrics, and durable call-commitment extraction | **Partial** | Hub | `src/app/api/cron/sync-recordings/route.ts`; `src/app/api/cron/extract-commitments/route.ts`; `src/app/api/recordings/route.ts`; `src/app/recordings/RecordingsView.tsx`; `src/app/api/recordings/route.test.ts`; `supabase/migrations/0024_commitment_extraction_tracking.sql`; `supabase/tests/migration/0024_commitment_extraction_tracking.test.sql` | The 2026-08-25 read-only production audit found the recording cursor still at 2026-08-08 after earlier schema errors. The protected recordings view now says when automatic HighLevel sync is fail-closed, so old calls are not presented as current. Keep scheduled extraction disabled until its separate cron activation proof. Only `metric_scope = 'performance'` may contribute to coaching, commitments, or reporting. |
