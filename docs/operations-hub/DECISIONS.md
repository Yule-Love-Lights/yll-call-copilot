...
| # | Decision | Ruling |
|---|---|---|
| F1 | Sequencing, supersedes R1 | HISTORICAL: ruled parallel tracks after joint Phase 0, with Quote Tool labor/time/scheduling independent from Hub advertising and office/install work. Decision 26 now supersedes F1 for Hub implementation and release order. |
...
| 16 | Production SMS/OTP provider and recovery/support process | RULED 2026-08-15: invite-only Supabase Phone Auth delivered through Twilio Verify; Turnstile; Naldo/Jason-only recovery with no employee self-service; maximum 30-day remembered session; password identities revoked at activation with Supabase-console break-glass. This remains the long-term target and is deferred by Decision 25. | Phase 0 identity |
| 17 | Versioned cross-repository contract | PARTIALLY COMPLETE: the normative Markdown, manifest, OpenAPI, and generated JSON Schema are published and mirrored byte-identically through `v1.5.0-draft`; authenticated cross-repository CI is active, while runtime deploy-skew health remains open | Phase 0 contract gate |
...
| 25 | Interim Operations Hub login and Phase 0 priority | RULED 2026-08-20: keep the existing invite-only Supabase email/password login. Defer phone OTP, Twilio Verify, Turnstile, owner recovery/reassignment, and password-identity revocation until a later activation decision. Keep phone auth false. Retain the paid staging Supabase project for migration, RLS, and persona verification, and prioritize those database, ledger, and authorization gates first. | Phase 0 sequencing / identity |
| 26 | Hub implementation and release order | RULED 2026-08-20: the Hub order is exactly Management, then Office, then Advertising, then Installer. Quote Tool contract, lifecycle-event, current-context, and command work may proceed in parallel with compatible Hub-owned foundations, but no dependent Hub surface or release may bypass its canonical-interface, authorization, hosted-persona, offline, or real-device gate. This supersedes F1 only for Hub sequencing. | Phase 0 and all four Hub releases |
| 27 | Hub modes, default landing, and switching | RULED 2026-08-20: the Hub has exactly four UI modes: Management, Office, Advertising, and Installer. Naldo/Jason always land in Management and enter department detail through Management drilldowns. Every other employee has an explicit primary home mode; the Hub never remembers the last-used mode as the next login default. A department switcher appears only for employees with multiple active department memberships. Outside a paid day it changes view only. During paid work it must use the canonical Quote Tool context switch. Naldo's final rule is to reject with review during an active Placement Run or open Installer job segment. Open-break final policy remains unresolved, so the current canonical safe default continues to reject with review. | Hub shell/navigation; amend canonical P16.2 before treating the active-run/job-segment rule as final contract authority |

Historical Track A/B/C labels elsewhere in this log identify the source work
domain only. They do not override Decision 26's current Hub sequence.
...
| Digest model | one unified / per-department | RULED 2026-08-06: FOUR DIGEST TYPES: Office, Advertising, Install, and Management. The first three combine department operations and attendance; Management is the cross-department owner/admin digest under Decision 23. Naldo and Jason receive all four. The QT morning ops digest and the hub coaching digest fold into this model. |
