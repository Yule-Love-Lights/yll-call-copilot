...
function fakeSupabase(role: string | null, scoreRow: Record<string, unknown> | null = null, transcriptRow: Record<string, unknown> | null = null) {
  fromMock = vi.fn((table: string) => {
    ...
    if (table === 'transcripts') {
      return { select: () => ({ eq: () => ({ maybeSingle: async () => ({ data: transcriptRow, error: null }) }) }) };
    }
...
describe('GET /api/coach/calls/[id] -- role gate', () => {
  ...
  it('lets a non-rep through to the full call plus its transcript', async () => {
    getSessionEmailMock.mockResolvedValue('naldo@yulelovelights.com');
    fakeClient = fakeSupabase(
      ...
      { raw_text: 'Rep: hi\n\nCustomer: hello', customer_name: 'Pat', outcome: 'booked', duration_seconds: 300, called_at: '2026-07-10T12:00:00Z' },
    );
