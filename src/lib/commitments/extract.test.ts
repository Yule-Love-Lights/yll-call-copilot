...
// throws-when-unconfigured path, and the validate/retry recovery. Same
// mocking style as src/lib/transcripts/extract.test.ts (mocks
// getClaudeClient via ../claude). #217
...
import { extractRawCommitments, EXTRACT_MODEL } from './extract';
...
describe('extractRawCommitments', () => {
  ...
  describe('happy path', () => {
    it('parses one send_quote commitment from "I\'ll send your quote today"', async () => {
      ...
      const result = await extractRawCommitments("Rep: I'll send your quote today. Customer: great, thanks.", 'Holiday Lighting');
    ...
    it('forces the emit_commitments tool and uses the configured model + token limit', async () => {
      ...
      await extractRawCommitments('a transcript', 'Event Lighting');
    ...
    it('instructs the model not to extract customer-made promises as rep commitments', async () => {
      ...
      await extractRawCommitments("Rep: sounds good. Customer: I'll think about it and call you back.", 'Holiday Lighting');
    ...
    it('returns an empty array for a transcript with no commitments, without error', async () => {
      ...
      const result = await extractRawCommitments('Rep: thanks for your time. Customer: no problem, bye.', 'Holiday Lighting');
    ...
    it('parses two distinct same-kind commitments as two separate entries', async () => {
      ...
      const result = await extractRawCommitments('a transcript with two photo promises', 'Holiday Lighting');
  ...
  describe('not configured', () => {
    it('throws when Claude is not configured', async () => {
      ...
      await expect(extractRawCommitments('x', 'Holiday Lighting')).rejects.toThrow(/not configured/i);
  ...
  describe('malformed response', () => {
    ...
    it('throws when Claude responds without a tool_use block', async () => {
      ...
      await expect(extractRawCommitments('x', 'Holiday Lighting')).rejects.toThrow(/did not return commitments/i);
    });
    ...
    it('retries once with the validation error, then succeeds', async () => {
      ...
      const result = await extractRawCommitments('x', 'Holiday Lighting');
    ...
    it('throws after exhausting retries on a persistently invalid kind', async () => {
      ...
      await expect(extractRawCommitments('x', 'Holiday Lighting')).rejects.toThrow(/incomplete commitments/i);
      expect(createMock).toHaveBeenCalledTimes(2);
    });
    ...
    it('retries once at a higher token ceiling when cut off, then throws if that also gets cut off', async () => {
      ...
      await expect(extractRawCommitments('x', 'Holiday Lighting')).rejects.toThrow(/token limit/i);
