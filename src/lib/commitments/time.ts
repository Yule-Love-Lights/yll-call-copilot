...
// CALL's wall-clock date (transcripts.called_at), never extraction time.
// Extraction runs 45-75 minutes after the call (src/lib/scoring/batch.ts's
// CANDIDATE_WINDOW/cron cadence is the same shape), so resolving against
// "now" would put a "3-ish" promise at the wrong hour and could roll it to
// the wrong calendar day near midnight. #217
//
// Same DST-correct Intl technique as src/lib/leads/callingHours.ts and
// src/lib/secondMile/decemberWindow.ts (read the zone's wall-clock fields
// directly via Intl.DateTimeFormat, no manual UTC-offset arithmetic) --
...
// promisedTimeLocal: "HH:MM" 24-hour local time in COMPANY_TIMEZONE, as
...
export function resolvePromisedAt(
  ...
  const { year, month, day } = localDateParts(COMPANY_TIMEZONE, calledAtDate);
